...
func NewVC(id string, typ []string, issuer string, credentialSubject map[string]interface{}) (*VC, error) {
	newVC := &VC{
		Context: []string{
			"https://www.w3.org/2018/credentials/v1", // 직접 참조하는거 아님. 사실상 스키마 체크 안함
			"https://www.w3.org/2018/credentials/v2",
		},
		Id:                id,
		Type:              typ,
		Issuer:            issuer,
		IssuanceDate:      time.Now().Format(time.RFC3339), //"2010-01-01T19:23:24Z", 발행날짜는 RFC3339 규격으로
...
func (vc *VC) GenerateJWT(verificationId string, pvKey *ecdsa.PrivateKey) string {
	...
	iat := t.Unix()                                     // issuer at
	nbf := iat                                          // before
	iss := vc.Issuer                                    // issuer
